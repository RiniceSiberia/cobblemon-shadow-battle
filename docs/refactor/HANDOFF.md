...
B6-back-button-symbols（2026-09-19）已完成：`BackButton` 的 19 个基线剩余安全声明已建立语义对应并改名，私有颜色常量接入实际绘制表达式；公开常量、方法签名和按钮几何保持。JDK21 离线 `clean build` 成功，9 个任务全部执行，259 项测试零失败；日志 `D:/workspace/gradle-back-button-symbols.log`。命名台账当前剩余 230 项，3487 个候选中 3257 个原声明键已消失，覆盖上限 93.40%。下一批处理 `ApiEvents.java`（18 项），再处理 `PlayerPortrait.java`（16 项）。真实客户端渲染、点击、远端联调、实体与 Mixin 以及 IDEA 验证仍未完成。
