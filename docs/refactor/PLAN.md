...
B6-message-catalogue-symbols（2026-09-19）：完成 Msg 的日志器、默认语言、资源目录、活动目录、语言解析、内置与外部读取、纯文本替换和组件组合声明语义改名。早期误名的已解析语言和两个输出变量同步修正，默认语言与内置目录常量恢复为实际引用。公开静态入口、默认 zh_cn、外部覆盖优先级、缺键回显、占位符解析和样式应用保持；真实外部目录与全部消息资源继续作为集成验收项。
...
完成 ClientSettings、OpenAuthScreenPayload 与 LeaderboardPayload 的内部命名改写；配置文件路径、认证序号和排行榜协议顺序保持。JDK21 离线 clean build 通过，259 项测试零失败。
