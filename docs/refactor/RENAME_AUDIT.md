...
2026-09-17 当前快照共有 3487 个候选，其中 1787 个原名仍存在，1700 个原声明键已消失，原声明键消失比例为 48.75%。该比例是待人工对应的上限，删除、迁入 Kotlin 和真正改名尚未逐项区分，不能作为最终 90% 验收结果。当前 Java 快照含 97 个文件、4277 个声明，javac 分析错误为 0。
...
已完成命名批次：`RoomLobbyScreen` 的纹理、布局、颜色、状态、输入、绘制、命中检测和表单流程声明完成语义改名；`LeaderboardScreen` 的纹理、布局、状态、榜单选择、画像缓存、实体姿态保存恢复、输入及绘制声明完成语义改名；`AuthScreen` 的认证模式、输入控件、验证码、提交状态、内部按钮和绘制声明完成语义改名。当前精确匹配不到这三个文件的基线安全原名。
