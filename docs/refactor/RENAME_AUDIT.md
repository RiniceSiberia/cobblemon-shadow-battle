...
2026-09-17 快照共有 3487 个候选，其中 2409 个原名仍存在，1078 个原声明键已消失，原声明键消失比例为 30.91%。该比例是待人工对应的上限，删除、迁入 Kotlin 和真正改名尚未逐项区分，不能作为最终 90% 验收结果。当前 Java 快照含 97 个文件、4277 个声明，javac 分析错误为 0。

剩余原名逐项记录在 `RENAME_REMAINING.csv`。优先按原名数量从高到低推进：

- `src/main/java/xiaocaoawa/minecraft/mod/cobblebattle/client/RoomLobbyScreen.java`：267 项
