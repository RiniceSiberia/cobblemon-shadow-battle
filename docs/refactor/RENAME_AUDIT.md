...
2026-09-17 当前快照共有 3487 个候选，其中 2330 个原名仍存在，1157 个原声明键已消失，原声明键消失比例为 33.18%。该比例是待人工对应的上限，删除、迁入 Kotlin 和真正改名尚未逐项区分，不能作为最终 90% 验收结果。当前 Java 快照含 97 个文件、4277 个声明，javac 分析错误为 0。
...
- `src/main/java/xiaocaoawa/minecraft/mod/cobblebattle/client/RoomLobbyScreen.java`：188 项
- `src/main/java/xiaocaoawa/minecraft/mod/cobblebattle/client/LeaderboardScreen.java`：180 项
...
已完成命名批次：`RoomLobbyScreen` 的纹理、布局、颜色、状态和输入字段共 79 个声明完成语义改名，原名剩余从 267 项降至 188 项。
