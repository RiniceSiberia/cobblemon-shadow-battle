...
class QueuePlayerPayloadTest {
    @Test
    fun `player object keeps protocol field order and text`() {
        val uuid = UUID.fromString("12345678-1234-5678-9abc-def012345678")
        assertEquals("{\"uuid\":\"12345678-1234-5678-9abc-def012345678\",\"name\":\"玩家\"}", QueuePlayerPayload.create(uuid, "玩家").toString())
    }
    ...
    fun `special name is encoded as JSON text`() {
        ...
        val name = " 名\n\"x\" "
        assertEquals(name, JsonParser.parseString(QueuePlayerPayload.create(uuid, name).toString()).asJsonObject.get("name").asString)
    }
}
